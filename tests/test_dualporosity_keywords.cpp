...
BOOST_AUTO_TEST_CASE(DeferredDualContinuumKeywordsStillFatal)
{
    ...
    for (const auto* kw : {"DUALPERM", "DZMATRIX", "DZMTRX", "DZMTRXV",
                           ...
                           "NMATRIX", "NMATOPTS"}) {
        auto it = blocked.find(kw);
        BOOST_REQUIRE_MESSAGE(it != blocked.end(),
                              std::string{kw} + " must stay in the unsupported-keyword list");
        BOOST_CHECK_MESSAGE(it->second.critical,
                            std::string{kw} + " must stay critical (hard error)");
    }
}
